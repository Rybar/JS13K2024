...
import MissileSplode from './gfx/MissileSplode.js';
...
function initGame(buffer) {
  ...
  function updateGame() {
    for (let i = missiles.length - 1; i >= 0; i--) {
      ...
      if (!missile.alive) {
        missiles.splice(i, 1);
        explosions.push(new MissileSplode(missile.x2, missile.y2, 200, 7));
      }
    }
    ...
    for (let i = enemyMissiles.length - 1; i >= 0; i--) {
      ...
      if (!enemyMissile.alive) {
        ...
        explosions.push(new MissileSplode(enemyMissile.x2, enemyMissile.y2, 200, 7));
      }
    }
